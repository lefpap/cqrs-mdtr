 ...
 */
public class BasicCqrsMediator implements CqrsMediator {
     ...
     */
    public BasicCqrsMediator(HandlerRegistry registry) {
    ...
    public <R, C extends Command<R>> R dispatch(C command) {
        ...
        CommandHandler<C, R> handler = registry.<C, R>getCommandHandler(command)
                .orElseThrow(
                        () -> new HandlerNotFoundException("No command handler registered for " + command.getClass()));
